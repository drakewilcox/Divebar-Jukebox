...
class PlaybackService:
    ...
    def get_next_transition(self, collection_id: str, session_id: str) -> Tuple[Optional[str], Optional[float], bool]:
        """
        ...
        """
        state = self.get_playback_state(collection_id, session_id)
        next_queue = self.queue_service.get_next_track(collection_id, session_id)
    ...
    def get_playback_state(self, collection_id: str, session_id: str) -> Optional[PlaybackState]:
        """Get playback state for a collection + session."""
        return self.db.query(PlaybackState).filter(
            PlaybackState.collection_id == collection_id,
            PlaybackState.session_id == session_id,
        ).first()

    def get_or_create_playback_state(self, collection_id: str, session_id: str) -> PlaybackState:
        """Get or create playback state for a collection + session."""
        state = self.get_playback_state(collection_id, session_id)
        if not state:
            state = PlaybackState(
    ...
    def play(self, collection_id: str, session_id: str) -> Optional[PlaybackState]:
        """Start or resume playback for this collection+session."""
        state = self.get_or_create_playback_state(collection_id, session_id)
        ...
        if not state.current_track_id:
            ...
            if next_queue:
                ...
                logger.warning(f"No tracks in queue for collection {collection_id} session {session_id}")
                return state

        state.is_playing = True
        self.db.commit()
        logger.info(f"Started playback for collection {collection_id} session {session_id}")
        return state

    def pause(self, collection_id: str, session_id: str) -> Optional[PlaybackState]:
        """Pause playback for this collection+session."""
        state = self.get_playback_state(collection_id, session_id)
        if state:
    ...
    def stop(self, collection_id: str, session_id: str) -> Optional[PlaybackState]:
        """Stop playback and reset position for this collection+session."""
        state = self.get_playback_state(collection_id, session_id)
        if state:
            state.is_playing = False
            state.current_position_ms = 0
    ...
    def skip(self, collection_id: str, session_id: str) -> Optional[PlaybackState]:
        """Skip to next track in queue for this collection+session."""
        state = self.get_or_create_playback_state(collection_id, session_id)
    ...
    def update_position(self, collection_id: str, session_id: str, position_ms: int) -> Optional[PlaybackState]:
        """Update current playback position for this collection+session."""
        state = self.get_playback_state(collection_id, session_id)
        if state:
    ...
    def set_volume(self, collection_id: str, session_id: str, volume: int) -> Optional[PlaybackState]:
        """Set playback volume for this collection+session."""
        state = self.get_or_create_playback_state(collection_id, session_id)
